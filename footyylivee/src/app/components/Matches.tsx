  ...
  return (
    <div className='grid grid-cols-3'>
        <div className='w-full flex items-center'>
            ...
            <p className='text-sm text-right text-white'>{data?.homeTeam?.name}</p>
        </div>
        <div className='px-2 m-auto flex justify-center items-center bg-slate-600 rounded-md'>
            {data?.status === 'FINISHED' ? (
                ...
                <p className='py-1 text-teal-400 text-xs'>{getDate} : {data?.status}</p>
            )}
        </div>
        <div className='w-full flex items-center justify-end'>
            <p className='text-sm text-right text-white'>{data?.awayTeam?.name}</p>
